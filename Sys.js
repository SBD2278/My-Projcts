...
client.on('messageCreate', message => {
    if (message.content === prefix + 'setr') {
...
if(!message.member.permissions.has("ADMINISTRATOR")) {
  message.reply({ content: '**you dont have permission to use this command**' });
  return;
}
        message.reply({
            content: `**${roles.members.map(member => `👀 | <@${e.user.id}>`).join('\n')}\ntheir number : \`${roles.members.size}\`**`
        })
}
...
// تعديل مهم
const prefix = "d!"; // حط البريفيكس اللي انت عايزه

const owner = ["523051807853772802","","","","",]; // اي دي الاونر اللي يقدر يتحكم في اوامر الاونر
...
client.user.setActivity(`${prefix}help`, {type:"PLAYING"}) // حط الحالة الي بدكياه
...
client.on("messageCreate", async function (message) {
  ...
  if (message.content.toLowerCase().startsWith(`${prefix}help`)) {
    ...
    const embed = new Discord.MessageEmbed()
      ...
      .setDescription(`
          ..  ${prefix}say \`بيخلي البوت يكتب\`
          ..  ${prefix}addemoji \`اضافه ايموجي للسيرفر\`
          ..  ${prefix}ban \`يدي بان\`
          ..  ${prefix}unban \`يفك البان\`
          ..  ${prefix}kick \`يعطي طرد\`
          ..  ${prefix}mute \`يعطي ميوت\`
          ..  ${prefix}unmute \`يفك الميوت\`
          ..  ${prefix}clear \`يمسح رسائل\`
          ..  ${prefix}hide \`يخلي الروم خاص\`
          .. ${prefix}show \`يخلي الروم يشوفها الجميع\`
          .. ${prefix}lock \`يقفل الروم\`
          .. ${prefix}server \`معرفة عدد السيرفرات الذي فيها البوت\`
          .. ${prefix}unlock \`يفتح الروم\`
          .. ${prefix}banner \`يجيب لك بنر احد\`
          .. ${prefix}timeout \`يعطي احد تايم اوت\`
          .. ${prefix}antibots on \`يمنع دخول البوتات\`
          .. ${prefix}antibots off \`يلغي منع دخول البوتات\`
          .. ${prefix}antibots \`اشوف البوتات اللي دخلت\`
          .. ${prefix}reaction-roles \`رياكشن رول\`
          .. ${prefix}tax \`يحسب لك ضريبه بروبوت\`
          .. ${prefix}role \`يدي احد رتبه\`
          .. ${prefix}role removed \`ازاله رتبه من احد\`
          .. ${prefix}hide-all \`اخفاء كل الرومات\`
          .. ${prefix}show-all \`اظهار كل الرومات\`
          .. ${prefix}come \`بيبعث رساله لاحد في الخاص\`
          .. ${prefix}send \`ترسل رسالة للشخص\`
          .. ${prefix}all-unban \`إلغاء حظر الكل\`
          .. ${prefix}ping \`يوريك بنق البوت\`
          .. ${prefix}user \`معلومات عنك انت\`
          .. ${prefix}avatar \`يجيب لك الافاتار حقك\`
          .. ${prefix}feedback \`تقييم شخص فقط في سيرفر الدعم\`
          .. ${prefix}avatar server \`يجيب افاتار السيرفر بس من دون تحميل\`
          .. ${prefix}setnick \`تغيير اسم احد\`
          .. ${prefix}invite \`يجيب لك انفايت للبوت حقك\`
          .. ${prefix}warn \`تدي انذار للشخص\`
...
// Code Servers
client.on('messageCreate', async message => {
if(message.content == prefix + 'server' || message.content == prefix + 'servers'){
        const serversNumber = new Discord.MessageEmbed()
        .setColor(message.guild.me.displayColor)
        .setAuthor({name : `${client.user.username}` , iconURL : client.user.displayAvatarURL({ dynamic: true })})
        .setDescription(`**عدد السيرفرات المنضم اليها :** ` + '**`' + client.guilds.cache.size +'`**')
        .setFooter(`Requested By ${message.author.tag}`, client.user.displayAvatarURL({ dynamic: true }))
        .setTimestamp()
        await message.channel.send({
            embeds : [serversNumber]
        })
    }
})
...
if(!message.member.permissions.has("BAN_MEMBERS")) return message.reply(`** 😕 You don't have permission (ban member) **`);
...
client.on("messageCreate", async message => {
   ...
   if (!message.member.permissions.has("KICK_MEMBERS")) return message.reply(`** 😕 You don't have permissions (kick member) **`);
...
if(!message.member.permissions.has("MANAGE_ROLES")) return message.reply(`** 😕 You don't have permission (MANAGE_ROLES)**`);
...
message.delete({timeout: 0})
    if(!message.channel.guild) return message.reply(`** This Command For Servers Only**`); 
     if(!message.member.permissions.has('MANAGE_MASSAGE')) return message.reply(`** 😕 You don't have permissions (MANAGE_MESSAGE) **`); 
...
if(message.author.bot || !message.guild) return;
if(!message.member.permissions.has('MANAGE_CHANNELS')) return message.reply(`** 😕 You don't have permissions (MANAGE_CHANNELS)**`);
...
if(message.author.bot || !message.guild) return;
if(!message.member.permissions.has('MANAGE_MASSEGE')) return message.reply(`** 😕 You don't have permissions **`);
...
client.on('messageCreate', async message => {
 if (!message.content.startsWith(prefix) || message.author.bot) return;
  if(!message.member.permissions.has('MANAGE_MESSAGE')) return message.reply(`** 😕 You don't have permissions **`); 
...
client.on("messageCreate", message => {
  if(message.content.startsWith(prefix + "feedback")){
    ...
    var fedbackchannel = message.guild.channels.cache.find(channel => channel.name === "feedback"); // اسم الروم
    const embed = new Discord.MessageEmbed()
...
client.on("messageCreate", message => {
if (message.content == "خط ببسي"){
message.reply("https://media.discordapp.net/attachments/1009756953955811460/1022845305554747413/20220821_174529.jpg")
}})
...
// تعديل مهم
// Code Suggestion
let sug = ["1031482767906721792","1031482707668107294","","",""]; // حط اي دي روم الاقتراحات
let line = "https://cdn.discordapp.com/attachments/1031482707668107294/1042777828736909342/523051807853772802.gif"; // حط رابط الخط
...
client.on('messageCreate', message => {
 ...
 if(!message.member.permissions.has('MANAGE_MASSEGE')) return message.reply(`** 😕 You don't have permissions (MANAGE_MESSAGES)**`); 
        ...
        let embed = new Discord.MessageEmbed()
        member.send(`> **Hay ${member}**\n\> **Please Come To ${channel}**`)

        let embed2 = new Discord.MessageEmbed()
        .setDescription(`> **Done Send To Member ${member}.**
                    
                         > **Please Wait !**`)
        .setColor("BLUE")
        .setTimestamp()
        .setFooter(`Dev By TalalAj`)
...
client.on("messageCreate", message => {
  ...
  .setFooter({ text: message.author.tag , iconURL: 
   message.author.displayAvatarURL({dynamic:true})})
  .setTitle("Info User")
  .addField('``الأسم``', ` ${message.author.tag} `, true)
  .addField('``الأيدي``', ` ${message.author.id} `, true)  
  .addField('``تاريخ الانشاء``', ` ${message.author.createdAt.toLocaleString()} `, true)
  .setTimestamp(); 
  message.channel.send({ embeds: [embed] })
  }
  });  
...
client.on("messageCreate", message => {
    if (message.content.startsWith(prefix + "setnick")) {
      if (!message.member.permissions.has("MANAGE_NICKNAMES")) return message.reply({ content: "You Dont Have Permission" })
...
if(!message.member.permissions.has("MANAGE_MASSEGE")) return message.channel.send({ content: `>>> \`\`\`You Don't have the permission ` });
 let args = message.content.split(" ").slice(1);
	...
	if (interaction.customId === 'apply') {
     ...
     if(interaction.member.roles.cache.some(r=>r.id == role)) return interaction.reply({content : "You are already a staff !" , ephemeral:true}) 
		...
		const modal = new ModalBuilder()
            ...
            .setRequired(true)
            .setPlaceholder("ادخل البيانات هنا")
			.setLabel("كيف تقدر تساعدنا السيرفر | وهل انت قد المسولية")
			.setStyle('PARAGRAPH')
            )
